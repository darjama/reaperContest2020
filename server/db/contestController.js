...
exports.resultsContest = function(req, res) {
  Contest.findOne({ resultdate: {$lt: new Date()} }, null, {sort: {resultdate: -1}}, function(err, contest) {
    if (err) return res.send(err);
    res.json(contest);
  });
};
...
exports.contestNames = function(req, res) {
  Contest.find( {}, ['contestid', 'contestlabel'], {sort: {'contestid': -1}}, function(err, contest) {
    if (err) return res.send(err);
    res.json(contest);
  });
};
