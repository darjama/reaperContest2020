...
const PlaylistNode = function (obj, prefix, prev = null, next = null) {
  (this.uri = `https://flac.reamixed.com/${obj.contestid}/${prefix}${(
    '0' + obj.mixnum.toString()
  ).slice(-2)}.flac`),
...
PlaylistMaker.prototype.addNode = function (obj, prefix) {
  let node = this.head;
  if (!this.head) {
    this.head = new PlaylistNode(obj, prefix);
    return;
  }
