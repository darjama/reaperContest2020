...
import {Table, Form, InputGroup, FormControl, Row, Col} from 'react-bootstrap';
...
const Comments = function({allComments, allEntries}) {
  ...
  allComments.forEach(a => { if (a.mixnum !== 'contestId') allCommenters.add(a.commenter)})

  const commentRows = allComments
  .filter(a => {
    if (a.mixnum === 'contestId') return false;
    let mixnum = a.mixnum === mixnumFilter || mixnumFilter === null;
    let commenter = a.commenter === commenterFilter || commenterFilter === null;
    let text = textFilter === '' || a.comment.toUpperCase().includes(textFilter.toUpperCase());
  ...
  return (
    <div style={{color: 'white'}}>
            ...
            <td>
              <Form>
                <Form.Group controlId="formGridMix">
                  ...
                  <Form.Control as="select" defaultValue="All" onChange={(e) => e.target.value === 'All' ? setMixnumFilter(null): setMixnumFilter(e.target.value)}>
                    <option>All</option>
                    {allEntries.sort((a,b) => a.mixnum - b.mixnum).map(a => <option key={a.mixnum}>{a.mixnum}</option>)}
                  </Form.Control>
                </Form.Group>
              </Form>
            </td>
            <td>
              <Form>
                <Form.Group controlId="formGridCommenter">
                  ...
                  onChange={(e) => e.target.value === 'All' ? setCommenterFilter(null): setCommenterFilter(e.target.value)}>
                    <option>All</option>
                    {[...allCommenters.values()].map(a => <option key={a}>{a}</option>)}
                  </Form.Control>
                </Form.Group>
              </Form>
            </td>
      ...
      <Table variant='dark' striped bordered hover size="sm"  style={{width:'95%'}}>
        <thead>
          <tr>
            <th>Mix #</th>
            <th>Commenter</th>
