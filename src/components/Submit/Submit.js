...
class Submit extends React.Component {
  ...
  checkFileSize(event) {
    ...
    for (var x = 0; x < files.length; x++) {
      if (files[x].size > size) {
        err +=
          files[x].name +
          ' is > 85MB. Please make sure you have not included any of the original media files, or contact us to deliver the file another way.\n';
      }
    }
  ...
  checkMimeType(event) {
    ...
    for (var x = 0; x < files.length; x++) {
      if (!type.includes(files[x].type)) {
        err +=
          files[x].name + ' is not a zip file. Please upload a zip file.\n';
      }
    }
  ...
  textChangeHandler(e) {
    ...
    if (e.target.id === 'email') {
      this.setState({
        emailWarning: this.validateEmail(e.target.value)
          ? ''
  ...
  clickHandler(e) {
    ...
    axios
      ...
      .then((res) => {
        this.setState({
          toastMessage: 'Upload Complete, thanks for entering!',
        });
        data.success = true;
      ...
      .catch((err) => {
        ...
        this.setState({
          toastMessage:
            'There was an error with your upload, please try again later or use the contact form to make arrangements to send a link to your file.',
        });
      });
  }
  ...
  render() {
    ...
    const early = `The contest hasn't started yet. Come back on  ${new Date(
      startdate
    ).toLocaleDateString('en-US', options)}.`;
    const late = `Sorry, you've missed the deadline for submissions. Come back to cast your vote on ${new Date(
      votestart
    ).toLocaleDateString('en-US', options)}.`;
    return (
      <React.Fragment>
        ...
        <Container style={{ width: '50%', color: 'white' }}>
          ...
          <Form>
            ...
            />
            <div
              disabled={this.state.sizeWarning === ''}
              style={{ color: 'red' }}
            >
              {this.state.sizeWarning}
            </div>
            <div
              disabled={this.state.typeWarning === ''}
              style={{ color: 'red' }}
            >
            ...
            <Button
              ...
              type='submit'
              disabled={
                !this.state.validFile || !(this.state.emailWarning === '')
              }
